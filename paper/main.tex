...
\begin{figure}[t]
  ...
  \caption{
    ROME accuracy recovery for four targeted corruption configurations. Recovery is positive in 3 of 4 configs (7$\to$1, 1$\to$7, 5$\to$6) and near-zero when the source-target classes are highly separable in hidden space (0$\to$8).
  }
  \label{fig:rome_recovery}
\end{figure}

\textbf{Multi-class targeted corruption.} We next measure whether ROME can \emph{repair} corrupted predictions by editing only the output layer. Recovery is positive in 3 of 4 targeted corruption configs (7$\to$1: $+0.116$, 1$\to$7: $+0.195$, 5$\to$6: $+0.160$) with consistent side effects ($\sim17$--25\%). The 0$\to$8 case shows near-zero recovery ($+0.014$). This is not explained by representation similarity (0 and 8 have the \emph{lowest} cosine similarity, $0.39$ vs $0.50$--$0.55$ for other pairs); rather, the edit magnitude for 0$\to$8 was smallest ($0.881$ vs $\sim1.0+$), suggesting the rank-one update was insufficient. This identifies a boundary condition for single-layer ROME: when classes are highly separable, the required edit is large and a single rank-one update may not suffice.
